 ...
 *
 * <p>Created by KitsuneAlex on 18/11/21.
 */
public class BinaryShaderObject extends NamedShaderObject implements ResourceManagerReloadListener {
    ...
    public BinaryShaderObject(
            String name,
            ResourceLocation asset,
            ShaderType type,
            BinaryType binaryType,
            String entryPoint,
            Collection<UniformPair> uniforms,
            Consumer<ConstantCache> specializationCallback) {
    ...
    public void alloc() {
        if (dirty || shaderId == -1) {
            ...
            ByteBuffer sourceBuffer =
                    BufferUtils.createByteBuffer(source.length).order(ByteOrder.nativeOrder());
    ...
    private byte[] getRawSource() {
        try {
            Resource resource = Minecraft.getInstance().getResourceManager().getResource(asset);
