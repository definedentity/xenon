...
/**
 * Note that when using the class as a vertex transformer, the vertices are assumed to be within the BB (x, y, z) ->
 * (x+1, y+1, z+1)
 */
public class LightMatrix implements IVertexOperation {
    ...
    public void sample(int i) {
        if ((sampled & 1 << i) == 0) {
            BlockPos bp =
                    new BlockPos(pos.getX() + (i % 3) - 1, pos.getY() + (i / 9) - 1, pos.getZ() + (i / 3 % 3) - 1);
