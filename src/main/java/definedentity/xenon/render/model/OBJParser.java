...
import definedentity.xenon.render.CCModel;
import definedentity.xenon.util.ResourceUtils;
import definedentity.xenon.vec.SwapYZ;
...
public class OBJParser {
    ...
    @Nullable private ResourceProvider provider;

    private VertexFormat.Mode vertexMode = VertexFormat.Mode.TRIANGLES;

    @Nullable private Transformation coordSystem;
    ...
    /**
     * Overload of {@link #coordSystem(Transformation)} specifying the {@link SwapYZ} coordinate system transformation.
     *
     * @return The same parser.
     */
     ...
     * @return The parsed models.
     */
    public static Map<String, CCModel> parse(
            ResourceProvider provider,
            ResourceLocation loc,
            VertexFormat.Mode vertexMode,
            @Nullable Transformation coordSystem,
            boolean ignoreMtl) {
        ...
        for (int ln = 0; ln < loadResource.size(); ln++) {
            ...
            switch (splits[0]) {
                ...
                case "usemtl" -> {
                    if (!ignoreMtl) {
                        ModelMaterial newMaterial = Objects.requireNonNull(matlib, "matlib definition not found")
                                .getMaterial(splits[1]);
