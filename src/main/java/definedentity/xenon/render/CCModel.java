...
import static definedentity.xenon.vec.Rotation.sideRotations;
import static net.covers1624.quack.util.SneakyUtils.unsafeCast;

import com.mojang.blaze3d.vertex.VertexFormat;
import definedentity.xenon.render.lighting.LC;
import definedentity.xenon.render.lighting.LightModel;
import definedentity.xenon.render.model.ModelMaterial;
...
import definedentity.xenon.vec.uv.UV;
import definedentity.xenon.vec.uv.UVTransformation;
import definedentity.xenon.vec.uv.UVTranslation;
import java.util.*;
...
public class CCModel implements IVertexSource, Copyable<CCModel> {
    ...
    @Nullable public ModelMaterial material() {
     ...
     * Gets an attribute.
     *
     * <p>If the model doesn't have this attribute, a new storage will be created and set.
     *
    ...
    /**
     * Ensires the specified {@link AttributeKey}'s index is available in the {@link #attributes} list.
     *
     ...
     *
     * <p>This will deeply copy all vertex data and attributes. {@link ModelMaterial} attributes will only be copied to
     * the destination model if the provided {@code srcpos} and {@code destpos} are {@code 0}.
     *
            ...
            for (int a = 0; a < src.attributes.size(); a++) {
                if (src.attributes.get(a) != null) {
                    ...
                    dst.attributes.set(
                            a,
                            key.copyRange(
                                    unsafeCast(src.attributes.get(a)), si, unsafeCast(dst.getOrAllocate(key)), di, 1));
                }
            }
    ...
    public CCModel generateSidedPart(int side1, int side2, Vector3 point, int srcpos, int destpos, int length) {
        return apply(
                new TransformationList(sideRotations[side1].inverse(), sideRotations[side2]).at(point),
                srcpos,
                destpos,
                length);
    }
     ...
     *
     * <p>This will deeply copy all vertices and attributes. The returned model will have the {@link ModelMaterial} of
     * the first provided Model.
     *
