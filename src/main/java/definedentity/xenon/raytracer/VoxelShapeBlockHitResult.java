 ...
 *
 * <p>Provides easy access to the distance, as well as the {@link IndexedVoxelShape} which the ray hit.
 *
 * <p>Created by covers1624 on 8/9/2016.
 */
public class VoxelShapeBlockHitResult extends SubHitBlockHitResult {
    ...
    public VoxelShapeBlockHitResult(
            Vector3 hit, Direction side, BlockPos pos, boolean isInside, IndexedVoxelShape shape, double dist) {
        super(hit, side, pos, isInside, shape.getData(), dist);
        this.shape = shape;
    }

    public VoxelShapeBlockHitResult(
            Vector3 hit, Direction side, boolean isInside, IndexedVoxelShape shape, double dist) {
        super(hit, side, BlockPos.ZERO, isInside, shape.getData(), dist);
        this.shape = shape;
    }

    protected VoxelShapeBlockHitResult(
            boolean isMissIn,
            Vec3 hit,
            Direction side,
            BlockPos pos,
            boolean isInside,
            IndexedVoxelShape shape,
            double dist) {
    ...
    public SubHitBlockHitResult withDirection(Direction newFace) {
        return new VoxelShapeBlockHitResult(
                getType() == Type.MISS, getLocation(), newFace, getBlockPos(), isInside(), shape, dist);
    }
