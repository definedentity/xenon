...
public class SubHitBlockHitResult extends BlockHitResult implements Comparable<SubHitBlockHitResult> {
    ...
    public SubHitBlockHitResult(
            Vector3 hitVec, Direction faceIn, BlockPos posIn, boolean isInside, Object data, double dist) {
        this(false, hitVec.vec3(), faceIn, posIn, isInside, data, dist);
    }

    public SubHitBlockHitResult(
            Vec3 hitVec, Direction faceIn, BlockPos posIn, boolean isInside, Object data, double dist) {
        this(false, hitVec, faceIn, posIn, isInside, data, dist);
    }

    protected SubHitBlockHitResult(
            boolean isMissIn,
            Vec3 hitVec,
            Direction faceIn,
            BlockPos posIn,
    ...
    public SubHitBlockHitResult withDirection(Direction newFace) {
        return new SubHitBlockHitResult(
                getType() == Type.MISS, getLocation(), getDirection(), getBlockPos(), isInside(), hitInfo, dist);
    }
